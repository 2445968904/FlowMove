...
void FGBWPHRootMotionSource_FromSKMesh::PrepareRootMotion(
	...
	const UCharacterMovementComponent& MoveComponent)
{
		...
		&& MovementTickTime > SMALL_NUMBER)
	{
		...
		const float CurrTime = (MontagePosition + SimulationTime * Rate);
		FTransform CurrRootMotion = MontageInstance->Montage->ExtractRootMotionFromTrackRange(PrevTime, CurrTime);

		const float RMScale = GetRootMotionScale();
		CurrRootMotion.SetTranslation(CurrRootMotion.GetTranslation() * RMScale);
		CurrRootMotion.SetRotation(CurrRootMotion.GetRotation() * RMScale);
		CurrRootMotion.NormalizeRotation();

		FVector Force = UKismetMathLibrary::TransformDirection(
			SKMesh->GetComponentTransform(),
			CurrRootMotion.GetLocation()) / MovementTickTime;
		...
		float rotationControlSpeed;
		if (GetRotationControlInfo(rotationControlSpeed) && Character.Controller)
		{
			FRotator DesiredRotation = Character.Controller->GetDesiredRotation();
			DesiredRotation.Pitch = 0.f;
			DesiredRotation.Yaw = FRotator::NormalizeAxis(DesiredRotation.Yaw);
			DesiredRotation.Roll = 0.f;

			const FRotator RotationNow = Character.GetRootComponent()->GetComponentRotation();
			DesiredRotation = UKismetMathLibrary::RInterpTo(RotationNow, DesiredRotation, SimulationTime, rotationControlSpeed);
			...
			->MoveComponent(
				...
				false,
				NULL,
				MoveComponent.MoveComponentFlags,
				ETeleportType::None);
		}

		const FTransform NewTransform(CurrRootMotion.GetRotation(),Force);
...
void UGBWPlayMontageCallbackProxy::Tick(float DeltaTime)
{
	if(!bIsEnd)
	{
		...
		if (bApplyRootMotion)
		{
			...
			if (PlayLength > 0 && 1-PlayState.Time/PlayLength <= PlayState.RootMotionEarlyEndTime)
			{
				DisableRMS();
			}
			else
			{
				if (bShouldApplyRootMotionNow)
				{
					TryActiveRMS();
				}
			}
...
UGBWPlayMontageCallbackProxy* UGBWPlayMontageCallbackProxy::CreateProxyObjectForPlayMontage(
	...
	float RootMotionEarlyEndTime,
	bool AllowRotationControl,
	float RotationControlSpeed,
	ERootMotionMode::Type RootMotionModeAfterEnd,
	FGBWPH_MontageNotifySetting MontageNotifySetting,
	UObject* ParameterObject)
{
	...
	Proxy->PlayMontage(
		...
		RootMotionEarlyEndTime,
		AllowRotationControl,
		RotationControlSpeed,
		RootMotionModeAfterEnd,
		MontageNotifySetting,
		ParameterObject);
	return Proxy;
}
	...
	float RotationControlSpeed)
{
	...
	RMS->Rate = Rate * MontageInstance->Montage->RateScale;
	RMS->AllowRotationControl = AllowRotationControl;
	RMS->RotationControlSpeed = RotationControlSpeed;
...
float UGBWPlayMontageCallbackProxy::GetPlayLength() const
{
	float res = -1.0f;
	
	if (PlayState.MontageToPlay && PlayState.PlayRate > 0)
	{
		int32 SectionIndex = PlayState.MontageToPlay->GetSectionIndex(PlayState.StartingSection);
		if (SectionIndex == INDEX_NONE)
		{
			SectionIndex = 0;
		}
		res = PlayState.MontageToPlay->GetSectionLength(SectionIndex);

		res = res / PlayState.PlayRate / PlayState.MontageToPlay->RateScale; 
	}
	...
	if (InSkeletalMeshComponent)
	{
		if (UAnimInstance* AnimInstance = InSkeletalMeshComponent->GetAnimInstance())
		{
			...
			if (bPlayedSuccessfully)
			{
				...
				RootMotionModeAfterEndCache = RootMotionModeAfterEnd;
				...
				PlayState.StartingPosition = StartingPosition;
				PlayState.StartingSection = StartingSection;
				PlayState.ApplyRootMotion = ApplyRootMotion;
				PlayState.RootMotionScale = RootMotionScale;
				PlayState.RootMotionAccumulateMode = RootMotionAccumulateMode;
				PlayState.RootMotionIgnoresGravity = RootMotionIgnoresGravity;
				PlayState.GravitationalAcceleration_ms2 = GravitationalAcceleration_ms2;
				PlayState.RootMotionEarlyEndTime = RootMotionEarlyEndTime;
				PlayState.AllowRotationControl = AllowRotationControl;
				PlayState.RotationControlSpeed = RotationControlSpeed;
				...
				RMSInfo.AllowRotationControl = AllowRotationControl;
				RMSInfo.RotationControlSpeed = RotationControlSpeed;

				MontageNotifyInfo = MontageNotifySetting;

				if (!MontageNotifyInfo.UseNotifyActiveRootMotion && PlayState.ApplyRootMotion)
				{
					bShouldApplyRootMotionNow = true;
				}
			}
		}
	}
	...
	if (bApplyRootMotion)
	{
		...
		if (!RMSInfo.CharacterMovement->GetRootMotionSource(RMSInfo.InstanceName))
		{
			if( ApplyRMS_BySKMesh(
				...
				RMSInfo.AllowRotationControl,
				RMSInfo.RotationControlSpeed))
			{
	...
	if (bApplyRootMotion)
	{
		...
		if (RMSInfo.CharacterMovement->GetRootMotionSource(RMSInfo.InstanceName))
		{
			if(DisableRMS_BySKMesh(
				...
				PlayState.SkeletalMeshComponent))
			{
				AnimInstancePtr->SetRootMotionMode(RootMotionModeAfterEndCache);
				return true;
			}
		}
	}
...
void UGBWPlayMontageCallbackProxy::OnNotifyBeginReceived(FName NotifyName, const FBranchingPointNotifyPayload& BranchingPointNotifyPayload)
{
	if (IsNotifyValid(NotifyName, BranchingPointNotifyPayload))
	{
		...
		if (MontageNotifyInfo.UseNotifyActiveRootMotion && NotifyName == MontageNotifyInfo.RootMotionActive)
		{
			bShouldApplyRootMotionNow = true;
		}
	}
...
void UGBWPlayMontageCallbackProxy::OnNotifyEndReceived(FName NotifyName, const FBranchingPointNotifyPayload& BranchingPointNotifyPayload)
{
	if (IsNotifyValid(NotifyName, BranchingPointNotifyPayload))
	{
		...
		if (MontageNotifyInfo.UseNotifyActiveRootMotion && NotifyName == MontageNotifyInfo.RootMotionActive)
		{
			bShouldApplyRootMotionNow = false;
			DisableRMS();
		}
	}
}
