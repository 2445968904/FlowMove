...
public:
	...
	UFUNCTION(BlueprintNativeEvent, Category = "GBW|Anim")
		bool GetRotationControlActive(bool& RotationControlActive, float& ControlSpeed);
};
USTRUCT(BlueprintType)
struct FGBWPH_MontageNotifySetting
{
	GENERATED_BODY()
	
	UPROPERTY(BlueprintReadWrite,EditAnywhere,Category = "Settings")
	bool UseNotifyActiveRootMotion = false;
	UPROPERTY(BlueprintReadWrite,EditAnywhere,Category = "Settings")
	FName RootMotionActive = "RootMotion";
	
	FGBWPH_MontageNotifySetting(){}
};
USTRUCT(BlueprintType)
struct GBWPOWERFULHIT_API FGBWPHRootMotionSource_FromSKMesh : public FRootMotionSource
{
	...
	UPROPERTY()
	float RotationControlSpeed = 5.0f;
	UPROPERTY()
	float Rate = 1.0f;
protected:
	UPROPERTY()
	...
	bool GetRotationControlInfo(float& ControlSpeed)
	{
		if (GetPHAnimInterfaceComponent())
		{
			bool res;
			float speed;
			if (IGBWBPI_MontagePlay::Execute_GetRotationControlActive(GetPHAnimInterfaceComponent(),res,speed))
			{
				ControlSpeed = speed;
				return res;
			}
		...
		ControlSpeed = RotationControlSpeed;
		return AllowRotationControl;
	}
...
struct FGBWPH_MontagePlayState
{
	...
	RootMotionAccumulateMode(ERootMotionAccumulateMode::Override),
	RootMotionIgnoresGravity(false),
	GravitationalAcceleration_ms2(-4.9f),
	RootMotionEarlyEndTime(0.3f),
	AllowRotationControl(false),
	RotationControlSpeed(5.0f)
	{}
};
USTRUCT(BlueprintType)
struct FGBWPH_RMSInfo_FromMesh
{
	...
	UPROPERTY(BlueprintReadWrite,Category = "Settings")
	FGBWPHRootMotionSource_FromSKMesh RMS = FGBWPHRootMotionSource_FromSKMesh();
...
class UGBWPlayMontageCallbackProxy : public UObject, public FTickableGameObject
{
	...
	static UGBWPlayMontageCallbackProxy* CreateProxyObjectForPlayMontage(
		...
		bool RootMotionIgnoresGravity = false,
		float GravitationalAcceleration_ms2 = -9.8f,
		float RootMotionEarlyEndTime = 0.3f,
		bool AllowRotationControl = false,
		float RotationControlSpeed = 5.0f,
		ERootMotionMode::Type RootMotionModeAfterEnd = ERootMotionMode::Type::RootMotionFromMontagesOnly,
		FGBWPH_MontageNotifySetting MontageNotifySetting = FGBWPH_MontageNotifySetting(),
		UObject* ParameterObject = nullptr);
...
private:
	...
	bool bShouldApplyRootMotionNow = false;
	
	TWeakObjectPtr<UAnimInstance> AnimInstancePtr;
	ERootMotionMode::Type RootMotionModeAfterEndCache = ERootMotionMode::Type::RootMotionFromMontagesOnly;
	...
	FGBWPH_MontageNotifySetting MontageNotifyInfo = FGBWPH_MontageNotifySetting();
